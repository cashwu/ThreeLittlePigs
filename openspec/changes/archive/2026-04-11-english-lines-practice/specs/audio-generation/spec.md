...
The system SHALL use a single JSON file (`lines.json`) as the authoritative data source for both English lines and Chinese translations. The file SHALL be an array of objects, each with `en` and `zh` fields. The audio generation script SHALL write this file, and `generate_audio.py` SHALL also generate `index.html` with the line data embedded inline as a JavaScript variable, so the webpage works when opened directly via `file://` without a server.
...
The system SHALL provide a Python script (`generate_audio.py`) that uses the `edge-tts` library to generate MP3 audio files for each English line from `lines.json`.
...
#### Scenario: Generate all English audio files

- **WHEN** the user runs `python generate_audio.py`
- **THEN** the script SHALL read all entries from `lines.json`, generate one MP3 per entry using edge-tts with an English voice, and save them to `audio/en_01.mp3` through `audio/en_09.mp3`
